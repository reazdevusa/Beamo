...
public class DatabaseService : IDatabaseService
{
    ...
    private static readonly string DbPath = Path.Combine(
        FileSystem.AppDataDirectory, "beamo.db3");
    ...
    public async Task InitializeAsync()
    {
        ...
        try
        {
            ...
            // sqlite-net-pcl supports max 4 types per CreateTablesAsync call
    ...
    public async Task ClearCompletedOperationsAsync(CancellationToken ct = default)
    {
        try
        {
            ...
            await db.Table<PendingOperation>()
                    .Where(static p => p.Status == "Completed")
                    .DeleteAsync();
        }
        catch (Exception ex)
        {
    ...
    public async Task<int> GetPendingOperationsCountAsync(CancellationToken ct = default)
    {
        try
        {
            var db = await GetDbAsync();
            return await db.Table<PendingOperation>()
                           .Where(static p => p.Status == "Pending")
                           .CountAsync();
        }
        catch (Exception ex)
        {
